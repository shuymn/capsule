...
- CI mirrors the same `task` commands used locally (`fmt:check`, `lint`, `package:check`, `release:test`, `test`, `build`).
- `task package:check` runs `cargo package --workspace --list --quiet --allow-dirty --locked` to catch missing or stale version requirements on workspace path dependencies before the release-plz candidate producer runs without changing `Cargo.lock`. This is an adapter constraint, not a crates.io publication policy; `release-plz.toml` disables publishing. Keep `capsule-cli` packageable because release-plz 0.3.159 skips manifest packages marked `publish = false`.
