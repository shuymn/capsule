...
- CI mirrors the same `task` commands used locally (`fmt:check`, `lint`, `package:check`, `test`, `build`).
- `task package:check` runs `cargo package --workspace --list --quiet --allow-dirty --locked` to catch missing or stale version requirements on workspace path dependencies before release automation runs without changing `Cargo.lock`.
